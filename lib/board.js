...
// Prevents any of `usernames` from holding a seat or queue spot on the
// other balcony at the same time as this one. Applies the same expiry
// logic as withBoard in memory (without persisting it) so a stale seat
// that just hasn't been lazily cleaned up yet doesn't block them.
async function assertNotOnOtherBalcony(client, usernames, balconyId) {
  const otherId = balconyId === 1 ? 2 : 1;
  const { rows } = await client.query('SELECT seats, seat_since, reservation, queue FROM board_state WHERE id = $1', [otherId]);
  const other = rows[0];
  if (!other) return;
  const board = { seats: other.seats, seatSince: other.seat_since, reservation: other.reservation, queue: other.queue, history: [] };
  cleanupExpiredSeats(board);
  const conflict = usernames.find((u) => board.seats.includes(u) || board.queue.includes(u));
  if (conflict) {
    const err = new Error(conflict + ' e gia occupato o in coda sul Balcone #' + otherId + '.');
    err.statusCode = 409;
    throw err;
  }
}
...
// members.length are locked. Throws 409/400 if the balcony isn't free or
// only occupied by people already in `members`, if a reservation is
// already active, or if there are too many members for this balcony's
// capacity.
function startReservation(board, type, members) {
  ...
  if (board.seats.some((s) => s && members.indexOf(s) === -1)) {
    const err = new Error('Il balcone deve essere libero, o occupato solo da chi hai selezionato, per attivare questa modalita.');
    err.statusCode = 409;
    throw err;
  }
  const n = board.seats.length;
...
// Drops history entries from a previous calendar month, so the occupancy
// data (and the chart built from it) resets each month. Persisted like
// everything else in withBoard's UPDATE, so it's a real DB deletion, not
// just a display filter.
function pruneOldHistory(board) {
  const now = new Date();
  board.history = board.history.filter((entry) => {
...
module.exports = { assignFromQueue, occupySeat, freeSeat, clearReservation, startReservation, withBoard, parseBalconyId, assertNotOnOtherBalcony, BALCONY_SEAT_COUNTS };
