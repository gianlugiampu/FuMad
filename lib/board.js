...
function clearReservation(board) {
  ...
  board.seatSince = [null, null, null, null];
  board.reservation = null;
}

// Reserves all 4 seats for `members` (owner first). Extra seats beyond
// members.length are locked. Throws 409 if the balcony isn't fully free
// or a reservation is already active.
function startReservation(board, type, members) {
  ...
  const now = new Date().toISOString();
  const seats = [LOCKED, LOCKED, LOCKED, LOCKED];
  members.forEach((name, i) => { seats[i] = name; });
  board.seats = seats;
  board.seatSince = [now, now, now, now];
...
// Locks the single board row, frees any seat past its individual TTL (or
...
async function withBoard(client, mutateFn) {
  ...
  try {
    const { rows } = await client.query('SELECT seats, seat_since, reservation, queue, history FROM board_state WHERE id = 1 FOR UPDATE');
    const row = rows[0];
    const board = {
      seats: row.seats,
    ...
    await client.query(
      'UPDATE board_state SET seats = $1, seat_since = $2, reservation = $3, queue = $4, history = $5 WHERE id = 1',
      [JSON.stringify(board.seats), JSON.stringify(board.seatSince), JSON.stringify(board.reservation), JSON.stringify(board.queue), JSON.stringify(board.history)]
    );
...
module.exports = { assignFromQueue, occupySeat, freeSeat, clearReservation, startReservation, withBoard };
